...
from kuralit.exceptions import ModelProviderError
from kuralit.models.base import Model
from kuralit.models.media import Audio, File, Image, Video
from kuralit.models.message import Citations, Message, UrlCitation
...
# Type alias for RunOutput - can be any object with metrics attribute
RunOutput = Any
...
class Gemini(Model):
    ...
    def invoke(
        ...
        try:
            if run_response and hasattr(run_response, 'metrics') and run_response.metrics:
    ...
    def format_function_call_results(
        ...
        if len(function_call_results) > 0:
            for result in function_call_results:
                ...
                # Handle tool_name attribute (may not exist in all cases)
                tool_name = getattr(result, 'tool_name', None) or getattr(result, 'name', 'unknown')
                combined_function_result.append({"tool_name": tool_name, "content": result.content})
                if hasattr(result, 'metrics'):
                    # Simple metrics combination
                    if hasattr(message_metrics, '__add__'):
                        message_metrics = message_metrics + result.metrics
                    else:
